...
# export TF_VAR_env= <your sdlc environment e.g. demo>
...
init.txt:
	...
	terraform remote config -backend=s3 -backend-config="bucket=$(STATEBUCKET)" -backend-config="key=$(PREFIX)/terraform-$(ENV).tfstate"
	echo "ran terraform remote config -backend=s3 -backend-config=\"bucket=$(STATEBUCKET)\" -backend-config=\"key=$(PREFIX)/terraform-$(ENV).tfstate\"" > ./init.txt
